...
window.onload = function() {
    ...
    logger.info('XOYO RC window loaded, browser = ', detectedBrowser, detectedVersion);
    ...
    if (typeof AudioContext !== 'undefined') {
        ...
        logger.warn('No XOYO RC audio context available, not showing vu-meter');
    }
    ...
    start_call_button.addEventListener('click', () => {
        ...
        createCall(target, 'AllUrGameRBelong2Us', logger, cloud_addr)
            .then((call: SlCall) => {
                ...
                let x = (document.getElementById('output') as any).captureStream();
                x.addTrack(local_stream.getAudioTracks()[0])
                call.dial(x);
            })
            .catch((error: any) => {
